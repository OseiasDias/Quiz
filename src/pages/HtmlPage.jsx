...
import perguntasHTML from "../data/PerguntasHtml.js";
...
export default function HtmlPage() {
  ...
  const perguntaAtual =  perguntasHTML.find(pergunta => pergunta.id === index);
  ...
  return (
    <section className="seccao-home text-white">
      <div className="container pt-5">
        <div className="row ">
          ...
          <a href="#" className="d-block itens arrajoItens">
              <FaHtml5 className="fs-3" style={{ color: "orange" }} />
              &nbsp;HTML
            </a>
          ...
          <div className="col-12 col-md-6 col-lg-6">
            <div className="mb-4">
              <span>Pergunta {perguntaAtual.id} / { perguntasHTML.length}</span>
          ...
          <div className="col-12 col-md-6 col-lg-6">
            {perguntaAtual.alternativas.map((alternativa, idx) => (
              <button
                key={idx}
                className={`text-start text-white d-block itens w-100 activada ${selected === idx ? 'selected' : ''}`}
      ...
      >
        <h2>{index ===  perguntasHTML.length ? "Fim do jogo!" : feedbackMessage}</h2>
        {index ===  perguntasHTML.length && (
          <div>
            <p>Acertos: {acertos}</p>
            <p>Erros: {erros}</p>
            {acertos >= 8 ? <h3>Parabéns! Você se saiu bem.</h3> : <h3>Pratique mais, você esteve mal!</h3>}
            <button onClick={handleCloseModal} className="itens text-white mx-auto d-block mt-3" style={{backgroundColor:"orange"}}>Fechar e voltar para o início</button>
          </div>
        )}
      </Modal>
 ...
 <button onClick={confirmExit} className="mx-2 itens text-white" style={{backgroundColor: "orange"}}>Sim</button>        <button onClick={cancelExit} className="mx-2 itens text-white" style={{backgroundColor: "gray"}}>Não</button>
      </Modal>
    </section>
  );
}
