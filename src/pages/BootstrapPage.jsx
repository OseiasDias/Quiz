...
import perguntasBootstrap from "../data/PerguntasBootstrap.js";
...
export default function BootstrapPage() {
  ...
  const perguntaAtual = perguntasBootstrap.find(pergunta => pergunta.id === index);
  ...
  const handleIncrementar = () => {
    if (selected !== null) {
      ...
      setTimeout(() => {
        ...
        if (index < perguntasBootstrap.length) {
  ...
  return (
    <section className="seccao-home text-white">
      <div className="container pt-5">
        <div className="row ">
          ...
          <a href="#" className="d-block itens arrajoItens">
              <FaBootstrap  className="fs-3" style={{ color: "#007bff" }} />
              &nbsp; Bootstrap
            </a>
          ...
          <div className="col-12 col-md-6 col-lg-6">
            <div className="mb-4">
              <span>Pergunta {perguntaAtual.id} / {perguntasBootstrap.length}</span>
          ...
          <div className="col-12 col-md-6 col-lg-6">
            {perguntaAtual.alternativas.map((alternativa, idx) => (
              <button
                key={idx}
                className={`text-start text-white d-block itens w-100 activada3 ${selected === idx ? 'selected' : ''}`}
      ...
      >
        <h2>{index === perguntasBootstrap.length ? "Fim do jogo!" : feedbackMessage}</h2>
        {index === perguntasBootstrap.length && (
          <div>
            <p>Acertos: {acertos}</p>
            <p>Erros: {erros}</p>
            {acertos >= 8 ? <h3>Parabéns! Você se saiu bem.</h3> : <h3>Pratique mais, você esteve mal!</h3>}
            <button onClick={handleCloseModal} className="itens text-white mx-auto d-block mt-3" style={{backgroundColor:"#007bff"}}>Fechar e voltar para o início</button>
          </div>
        )}
      </Modal>
 ...
 <button onClick={confirmExit} className="mx-2 itens text-white" style={{backgroundColor: "#007bff"}}>Sim</button>        <button onClick={cancelExit} className="mx-2 itens text-white" style={{backgroundColor: "gray"}}>Não</button>
      </Modal>
    </section>
  );
}
