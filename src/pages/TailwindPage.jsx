...
import perguntasTailwind from "../data/PerguntasTailwind.js";
...
export default function TailwindPage() {
  ...
  const perguntaAtual = perguntasTailwind.find(pergunta => pergunta.id === index);
  ...
  const handleIncrementar = () => {
    if (selected !== null) {
      ...
      setTimeout(() => {
        ...
        if (index < perguntasTailwind.length) {
  ...
  return (
    <section className="seccao-home text-white">
      <div className="container pt-5">
        <div className="row ">
          ...
          <a href="#" className="d-block itens arrajoItens">
              <RiTailwindCssFill className="fs-3" style={{ color: "#fff" }} />
              &nbsp;Tailwind Css
            </a>
          ...
          <div className="col-12 col-md-6 col-lg-6">
            <div className="mb-4">
              <span>Pergunta {perguntaAtual.id} / {perguntasTailwind.length}</span>
          ...
          <div className="col-12 col-md-6 col-lg-6">
            {perguntaAtual.alternativas.map((alternativa, idx) => (
              <button
                key={idx}
                className={`text-start text-white d-block itens w-100 activada0 ${selected === idx ? 'selected' : ''}`}
            ...
            <ButtonConfirmar 
              ...
              useP="w-100 itens text-black fw-bold" 
              useCor={"#fff"} 
              useText="Confirmar" 
            />
          </div>
      ...
      >
        <h2>{index === perguntasTailwind.length ? "Fim do jogo!" : feedbackMessage}</h2>
        {index === perguntasTailwind.length && (
          <div>
            <p>Acertos: {acertos}</p>
            <p>Erros: {erros}</p>
            {acertos >= 8 ? <h3>Parabéns! Você se saiu bem.</h3> : <h3>Pratique mais, você esteve mal!</h3>}
            <button onClick={handleCloseModal} className="itens text-black mx-auto d-block mt-3" style={{backgroundColor:"#fff"}}>Fechar e voltar para o início</button>
          </div>
        )}
      </Modal>
 ...
 <button onClick={confirmExit} className="mx-2 itens text-black" style={{backgroundColor: "#fff"}}>Sim</button>        <button onClick={cancelExit} className="mx-2 itens text-white" style={{backgroundColor: "gray"}}>Não</button>
      </Modal>
    </section>
  );
}
