...
import perguntasSass from "../data/PerguntasSass.js";
...
export default function SassPage() {
  ...
  const perguntaAtual = perguntasSass.find((pergunta) => pergunta.id === index);
  ...
  return (
    <section className="seccao-home text-white">
        ...
        <div className="row ">
          <div className="header d-flex justify-content-between">
            <a href="#" className="d-block itens arrajoItens">
              <SiSass className="fs-3" style={{ color: "#28a745" }} />
              &nbsp;Sass Pré-pocessadores
            </a>
          ...
          <div className="col-12 col-md-6 col-lg-6">
            <div className="mb-4">
              <span>
                Pergunta {perguntaAtual.id} / {perguntasSass.length}
              </span>
          ...
          <div className="col-12 col-md-6 col-lg-6">
            {perguntaAtual.alternativas.map((alternativa, idx) => (
              <button
                key={idx}
                className={`text-start text-white d-block itens w-100 activada6 ${
                  selected === idx ? "selected" : ""
                }`}
                onClick={() => setSelected(idx)}
              >
                <span className="colorirLetra">
                  {alternativa.letra}&nbsp;&nbsp;
                </span>
                {alternativa.texto}
              </button>
      ...
      <Modal isOpen={modalIsOpen} onClose={() => setModalIsOpen(false)}>
        <h2>
          {index === perguntasSass.length ? "Fim do jogo!" : feedbackMessage}
        </h2>
        {index === perguntasSass.length && (
          <div>
            <p>Acertos: {acertos}</p>
            <p>Erros: {erros}</p>
            {acertos >= 8 ? (
              <h3>Parabéns! Você se saiu bem.</h3>
            ) : (
              <h3>Pratique mais, você esteve mal!</h3>
            )}
            <button
              onClick={handleCloseModal}
              className="itens text-white mx-auto d-block mt-3"
              style={{ backgroundColor: "#28a745" }}
            >
              Fechar e voltar para o início
            </button>
          </div>
        )}
      </Modal>
      ...
      <Modal isOpen={confirmExitModalIsOpen} onClose={cancelExit}>
        <h2>Você tem certeza que deseja sair?</h2>
        <button
          onClick={confirmExit}
          className="mx-2 itens text-white"
          style={{ backgroundColor: "#28a745" }}
        >
          Sim
        </button>{" "}
